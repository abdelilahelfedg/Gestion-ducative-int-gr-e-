...
<form class="form" method="POST" name="module" enctype="multipart/form-data">
  ...
  <div class="form-paragraph">
    ...
    <select name="TP3" class="input" id="floatingSelect" aria-label="Floating label select example">
    
    <option value="" disabled selected>Choisissez Le groupe de TP</option>
    <?php foreach ($data as $nombre): ?>
        <option value="<?php echo $nombre; ?>"><?= $nombre;?></option>
    <?php endforeach; ?>
    </select>
  </div>
  <button type="submit" name="TP2">Valider</button>
...
<div class="container mt-3">
    <div class="d-flex flex-column align-items-end">
        <div class="me-2">
            <a href="<?= ROOT . "/" . "HomeProf" ?>">
                <button class="">Retour</button>
            </a>
        </div>
        <div class="mt-2">
            <form action="<?= ROOT."/"."AbsenceControl"?>" method="POST" name="TPP">
                <button style="display:block;" name="retour_en_arriere" class="">Retour en arriere</button>
            </form>
        </div>
    </div>
</div>
